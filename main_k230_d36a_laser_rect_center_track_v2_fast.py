...
# ============================================================
# 激光点闭环配置（RGB565 + find_blobs，避免Python逐像素扫描）
# ============================================================
LASER_IO_PIN = 35
...
LASER_CFG = {
    # 红色激光LAB阈值，基于此前RGB565链路
    "threshold": (18, 88, 30, 127, 5, 127),
    "pixels_min": 2,
    "area_min": 2,
    "area_max": 220,
    "max_w": 24,
...
class LaserTTL35:
    def __init__(self):
        self.pin = make_gpio_output(LASER_IO_PIN, 0 if LASER_ACTIVE_LEVEL else 1)
        self.enabled = False

    def on(self):
        write_pin(self.pin, LASER_ACTIVE_LEVEL)
        self.enabled = True

    def off(self):
        write_pin(self.pin, 0 if LASER_ACTIVE_LEVEL else 1)
        self.enabled = False
...
class FastLaserTracker:
    ...
    @staticmethod
    def _blob_value(blob, name, index, default=0):
        try:
    ...
    def detect(self, img):
        ...
        candidates = []
        for blob in blobs:
            x = float(self._blob_value(blob, "x", 0, 0))
            y = float(self._blob_value(blob, "y", 1, 0))
            w = float(self._blob_value(blob, "w", 2, 0))
            h = float(self._blob_value(blob, "h", 3, 0))
            pixels = float(self._blob_value(blob, "pixels", 4, max(1.0, w * h)))
            cx = float(self._blob_value(blob, "cx", 5, x + 0.5 * w))
            cy = float(self._blob_value(blob, "cy", 6, y + 0.5 * h))
            area = max(1.0, w * h)

            if area < LASER_CFG["area_min"] or area > LASER_CFG["area_max"]:
                continue
            if w <= 0 or h <= 0 or w > LASER_CFG["max_w"] or h > LASER_CFG["max_h"]:
                continue

            aspect = max(w, h) / max(1.0, min(w, h))
            density = pixels / area
            if aspect > LASER_CFG["max_aspect"] or density < LASER_CFG["min_density"]:
                continue
            ...
            if self.position is not None:
                ...
                if dist > LASER_CFG["gate_locked_px"]:
                    continue
                score = dist + 0.02 * abs(area - 10.0)
            else:
                # 未锁定时优先选择小而紧凑的光点
                score = 0.03 * abs(area - 10.0) + 2.0 * (1.0 - min(1.0, density))
        ...
        if not candidates:
            self.lost_frames += 1
            if self.lost_frames > int(LASER_CFG["max_lost_frames"]):
                self.position = None
            return None
        ...
        if self.position is None:
            ...
            self.position = (
                self.position[0] + a * (cx - self.position[0]),
                self.position[1] + a * (cy - self.position[1]),
            )

        self.lost_frames = 0
        return (
            detect_to_logical_x(self.position[0]),
            detect_to_logical_y(self.position[1]),
        )
...
class UARTLink:
    ...
    def send_display_help(self):
        ...
        lines = (
            ...
            "CH1 rect_minus_laser_x CH2 rect_minus_laser_y CH3 x_target_hz CH4 y_target_hz",
...
class K230RectangleStepperTrackerV31:
    def __init__(self):
        ...
        self.uart = UARTLink()
        self.laser_ttl = LaserTTL35()
        self.laser_tracker = FastLaserTracker()
        self.last_laser = None
        self.last_control_err_x = 0.0
        self.last_control_err_y = 0.0
    ...
    def update_tracking_control(self, result, dt):
        if self.estop or result["center"] is None or self.last_laser is None:
            self.stop_motion(hard=True)
            return

        rect_x, rect_y = result["center"]
        laser_x, laser_y = self.last_laser

        # 闭环误差：矩形中心 - 激光点
        err_x = float(rect_x - laser_x)
        err_y = float(rect_y - laser_y)
        self.last_control_err_x = err_x
        self.last_control_err_y = err_y

        # 使用矩形速度作为前馈；激光点本身由云台运动产生
    ...
    def send_plot(self, result):
        ...
        if result["center"] is None:
            ...
            if self.last_laser is None:
                ...
                err_x = float(tx - self.last_laser[0])
                err_y = float(ty - self.last_laser[1])
        self.uart.send(
    ...
    def draw(self, img, result):
        ...
        if self.last_laser is not None:
            lx, ly = self.last_laser
            lx_det = int(round(logical_to_detect_x(lx)))
            ly_det = int(round(logical_to_detect_y(ly)))
            img.draw_circle(lx_det, ly_det, 4, color=(255, 0, 255), thickness=2)
    ...
    def run(self):
        ...
        try:
            print("=" * 76)
            print("K230 D36A LASER-TO-RECT CENTER TRACKER V2 FAST")
            ...
            print("EN1/EN2 tied to D36A board 5V; GPIO35 laser TTL enabled")
            print("=" * 76)
            self.startup_motor_self_test()
            ...
            while True:
                ...
                self.last_laser = self.laser_tracker.detect(laser_img)
                ...
                if self.tracking_enabled and not self.estop:
                    if (
                        ...
                        and result["center"] is not None
                        and self.last_laser is not None
                    ):
                        self.update_tracking_control(result, dt)
                    else:
                        self.stop_motion(hard=True)
