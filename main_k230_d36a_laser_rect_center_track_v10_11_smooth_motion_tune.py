# -*- coding: utf-8 -*-
"""
K230 + D36A 二维步进云台 矩形中心-激光点闭环 v10.11
平滑运动细调版 / Smooth-motion tracking tune
...
MEASURE_FRESH_MS = 190           # 两个测量都在此时间内有效才全速TRACK
MEASURE_HOLD_MS = 820           # 短时漏检：保留误差但衰减目标频率
MEASURE_LOST_MS = 1550           # 超过该时间：目标频率归零，Ramp减速停
...
CTRL_CFG = {
    ...
    # v10.11：平滑运动细调。略增靶心静默区，避免1~3px噪声反复触发低速脉冲。
    "x_stop_enter_px": 0.75,
    "x_stop_exit_px": 1.90,
    "y_stop_enter_px": 0.85,
    "y_stop_exit_px": 2.10,
    ...
    # v10.11：误差超过约2px再具备起步资格，降低靶心附近碎动。
    "x_start_error_px": 2.00,
    "y_start_error_px": 2.20,

    # v10.11：微调触发更柔和。只补足小输出，不钳制大误差下的正常PD速度。
    # 原始误差需超过门槛并连续同向确认，才给低速精修脉冲。
    "x_micro_enable_px": 3.20,
    "y_micro_enable_px": 3.50,
    "x_micro_hz": 9.0,
    "y_micro_hz": 8.0,
    "micro_same_sign_px": 5.00,
    # v10.11：微调必须连续同方向确认。降低micro_hz并增加确认帧，减少低速碎抖。
    "micro_confirm_frames": 4,
    "micro_strong_px": 13.0,
    "micro_reset_px": 1.70,

    # v10.11：靶心偏差校准。控制误差 = 矩形中心 - 激光中心 + aim_offset。
    # 视觉日志接近0但肉眼仍偏靶心时，用串口微调：
    # [slider,aimOffsetX,正负像素] / [slider,aimOffsetY,正负像素]
    "aim_offset_x": 0.0,
    "aim_offset_y": 0.0,

    # v10.11：持续小偏差精修变轻。小误差长期同方向存在时，只给很小补偿，优先保证运动平滑；
    # 大误差、换向、丢目标时自动泄放，避免恢复 v10.6 的碎抖。
    "fine_enable_x_px": 1.80,
    "fine_enable_y_px": 2.00,
    "fine_region_x_px": 8.0,
    "fine_region_y_px": 9.0,
    "fine_i_gain_x": 1.35,
    "fine_i_gain_y": 1.20,
    "fine_i_cap_x_hz": 4.5,
    "fine_i_cap_y_hz": 4.0,
    "fine_i_leak_s": 1.35,
    "fine_i_reset_px": 18.0,
    "fine_confirm_frames": 6,

    # v10.11：小误差跨零不快速跟随，避免滤波误差被单帧噪声拉反。
    "cross_fast_px": 15.0,
    "cross_fast_alpha": 0.52,
    "small_recede_px": 13.0,
    ...
    # v10.11：测量异常跳变保护更保守。第一帧大跳变先当作可疑测量，只衰减输出；
    # 连续确认后才接受，避免单帧误识别导致云台乱甩。
    "raw_jump_reject_px": 52.0,
    "raw_jump_confirm_frames": 4,
    "outlier_decay": 0.12,
    "outlier_cap_x_hz": 70.0,
    "outlier_cap_y_hz": 60.0,

    # v10.11：目标频率二级限斜率降低。StepperAxis已有Ramp，这里再限制控制目标的突然变化，
    # 让 target_hz 本身连续，减少重捕获/短时漏检后的冲击。
    "x_target_slew_hz_s": 3900.0,
    "y_target_slew_hz_s": 3400.0,
    "x_zero_cross_stop_px": 3.20,
    "y_zero_cross_stop_px": 3.50,

    # v10.11：最终运动决策层。核心原则不变，静默区略增，优先抑制靶心附近碎动。
    # 若 raw error 已经连续很小，或者控制方向与 raw error 相反，直接禁止微动，
    # 解决 filtered error / fine_i / target_slew 残留导致的无意义抖动。
    "x_quiet_px": 1.75,
    "y_quiet_px": 1.95,
    "quiet_confirm_frames": 3,
    "x_opposite_stop_px": 10.0,
    "y_opposite_stop_px": 10.0,
    "hold_axis_stop_px": 8.0,
    "hold_force_zero_ms": 340,
...
class FixedRatePDController:
    def __init__(self):
        ...
        self.micro_x_sign = 0
        self.micro_y_sign = 0
        self.micro_x_count = 0
        self.micro_y_count = 0
        self.fine_i_x = 0.0
        self.fine_i_y = 0.0
        self.fine_x_sign = 0
        self.fine_y_sign = 0
        self.fine_x_count = 0
        self.fine_y_count = 0
        self.quiet_x_count = 0
        self.quiet_y_count = 0
    ...
    def _filter_error(self, raw_x, raw_y):
        ...
        # v10.11：小误差区的跨零多数来自测量噪声，不再用0.82快速拉反；
        # 只有误差明显变大/目标快速远离时才快速跟随。
        if raw_x * self.err_x <= 0.0:
            if abs(raw_x) >= CTRL_CFG["cross_fast_px"] or abs(self.err_x) >= CTRL_CFG["cross_fast_px"]:
                ax = max(ax, CTRL_CFG["cross_fast_alpha"])
            else:
                ax = min(ax, CTRL_CFG["err_alpha_near"])
        elif abs(raw_x) > abs(self.err_x) + CTRL_CFG["small_recede_px"]:
            ax = max(ax, CTRL_CFG["cross_fast_alpha"])
        if raw_y * self.err_y <= 0.0:
            if abs(raw_y) >= CTRL_CFG["cross_fast_px"] or abs(self.err_y) >= CTRL_CFG["cross_fast_px"]:
                ay = max(ay, CTRL_CFG["cross_fast_alpha"])
            else:
                ay = min(ay, CTRL_CFG["err_alpha_near"])
        elif abs(raw_y) > abs(self.err_y) + CTRL_CFG["small_recede_px"]:
            ay = max(ay, CTRL_CFG["cross_fast_alpha"])
    ...
    def _axis_pd(self, axis_name, e, dt):
        ...
        # 小误差跨零后只在约2px内短暂停；v10.11继续缩小跨零停区，减少最终偏差。
    ...
    def _apply_micro_nudge(self, axis_name, target, raw_e, filt_e):
        # v10.11：微调只补足小输出，并要求同方向连续确认。
        # 目的：保留靶心精修能力，同时抑制 1~7px 噪声导致的±micro_hz反复抖动。
        if axis_name == "x":
            enable_px = CTRL_CFG["x_micro_enable_px"]
            micro_hz = CTRL_CFG["x_micro_hz"]
            prev_sign = self.micro_x_sign
            prev_count = self.micro_x_count
        else:
            enable_px = CTRL_CFG["y_micro_enable_px"]
            micro_hz = CTRL_CFG["y_micro_hz"]
            prev_sign = self.micro_y_sign
            prev_count = self.micro_y_count
        ...
        if ae < CTRL_CFG["micro_reset_px"]:
            sign = 0
            count = 0
        else:
            sign = 1 if raw > 0.0 else -1
            count = prev_count + 1 if sign == prev_sign else 1

        if axis_name == "x":
            self.micro_x_sign = sign
            self.micro_x_count = count
        else:
            self.micro_y_sign = sign
            self.micro_y_count = count

        if ae < enable_px:
            return target

        # raw 和滤波误差方向不一致时，说明正在跨中心或受噪声扰动，小误差区不追。
        if raw * filt < 0.0 and ae < CTRL_CFG["micro_same_sign_px"]:
            return 0.0

        # 对小误差微调，必须连续同方向确认；大误差立即允许正常PD处理。
        if ae < CTRL_CFG["micro_strong_px"] and count < int(CTRL_CFG["micro_confirm_frames"]):
            return 0.0

        # 正常PD已经给出足够输出时，不再干预速度。
        if abs(target) >= micro_hz:
            return target

        if sign == 0:
            return 0.0
        return float(sign) * micro_hz

    def _apply_precision_trim(self, axis_name, target, raw_e, dt):
        # v10.11：对“长期同方向小偏差”做低速积分补偿。
        # 这不是普通I项：只在小范围内、有效测量、连续同向时启用；误差大/换向/过小时泄放。
        if axis_name == "x":
            enable_px = CTRL_CFG["fine_enable_x_px"]
            region_px = CTRL_CFG["fine_region_x_px"]
            gain = CTRL_CFG["fine_i_gain_x"]
            cap = CTRL_CFG["fine_i_cap_x_hz"]
            min_hz = CTRL_CFG["x_min_hz"]
            fine_i = self.fine_i_x
            prev_sign = self.fine_x_sign
            count = self.fine_x_count
        else:
            enable_px = CTRL_CFG["fine_enable_y_px"]
            region_px = CTRL_CFG["fine_region_y_px"]
            gain = CTRL_CFG["fine_i_gain_y"]
            cap = CTRL_CFG["fine_i_cap_y_hz"]
            min_hz = CTRL_CFG["y_min_hz"]
            fine_i = self.fine_i_y
            prev_sign = self.fine_y_sign
            count = self.fine_y_count

        raw = float(raw_e)
        target = float(target)
        ae = abs(raw)
        leak = max(0.0, 1.0 - float(CTRL_CFG["fine_i_leak_s"]) * max(0.004, float(dt)))

        if ae > CTRL_CFG["fine_i_reset_px"]:
            fine_i = 0.0
            sign = 0
            count = 0
        elif ae < enable_px:
            ...
            sign = 1 if raw > 0.0 else -1
            count = count + 1 if sign == prev_sign else 1
            if ae <= region_px and count >= int(CTRL_CFG["fine_confirm_frames"]):
                fine_i += gain * raw * max(0.004, float(dt))
                fine_i = clamp(fine_i, -cap, cap)
            else:
                fine_i *= leak

        if axis_name == "x":
            self.fine_i_x = fine_i
            self.fine_x_sign = sign
            self.fine_x_count = count
        else:
            self.fine_i_y = fine_i
            self.fine_y_sign = sign
            self.fine_y_count = count

        # 若PD/微调已经给了足够速度，不额外干预；只补偿“卡在小偏差不动”的情况。
        if abs(target) >= min_hz:
            return target

        corrected = target + fine_i
        if 0.0 < abs(corrected) < min_hz and count >= int(CTRL_CFG["fine_confirm_frames"]):
            corrected = min_hz if corrected > 0.0 else -min_hz
        return corrected

    def _reset_axis_decision_memory(self, axis_name):
        # 清掉会导致中心附近继续微动的控制记忆。
        if axis_name == "x":
            self.der_x = 0.0
            self.micro_x_sign = 0
    ...
    def _apply_final_motion_decision(self, axis_name, target, raw_e):
        # v10.11最终仲裁：filtered error、微分、fine_i、target_slew 都不能违背当前 raw error。
        raw = float(raw_e)
        target = float(target)
        ar = abs(raw)
        if axis_name == "x":
            quiet_px = CTRL_CFG["x_quiet_px"]
            opposite_stop_px = CTRL_CFG["x_opposite_stop_px"]
            if ar <= quiet_px:
                self.quiet_x_count += 1
            else:
                self.quiet_x_count = 0
            quiet_count = self.quiet_x_count
        else:
            quiet_px = CTRL_CFG["y_quiet_px"]
            opposite_stop_px = CTRL_CFG["y_opposite_stop_px"]
            if ar <= quiet_px:
                self.quiet_y_count += 1
            else:
                self.quiet_y_count = 0
            quiet_count = self.quiet_y_count

        # 靶心附近连续稳定：强制静默，不允许 fine_i / min_hz / slew 残留继续推电机。
        if quiet_count >= int(CTRL_CFG["quiet_confirm_frames"]):
            self._reset_axis_decision_memory(axis_name)
            return 0.0

        # 小误差区域，若控制方向与当前测量误差相反，说明是在追滤波残留或上一拍指令。
        # 这种运动就是典型无意义抖动，直接截断。
        if ar <= opposite_stop_px and raw * target < 0.0:
            self._reset_axis_decision_memory(axis_name)
            return 0.0
    ...
    def update(self, rect_pos, rect_age_ms, laser_pos, laser_age_ms, dt, laser_edge_pos=None):
        ...
        if valid:
            raw_x = float(rect_pos[0] - laser_pos[0]) + float(CTRL_CFG["aim_offset_x"])
            raw_y = float(rect_pos[1] - laser_pos[1]) + float(CTRL_CFG["aim_offset_y"])
            ...
            tx = self._apply_micro_nudge("x", tx, raw_x, self.err_x)
            ty = self._apply_micro_nudge("y", ty, raw_y, self.err_y)
            tx = self._apply_precision_trim("x", tx, raw_x, dt)
            ty = self._apply_precision_trim("y", ty, raw_y, dt)
            tx = self._apply_final_motion_decision("x", tx, raw_x)
            ty = self._apply_final_motion_decision("y", ty, raw_y)
            ...
            # v10.11：HOLD只用于目标快速运动时的短暂桥接；靶心附近漏检不允许继续微动。
            if abs(self.err_x) <= CTRL_CFG["hold_axis_stop_px"] or age >= CTRL_CFG["hold_force_zero_ms"]:
                tx = 0.0
                self.slew_target_x = 0.0
            if abs(self.err_y) <= CTRL_CFG["hold_axis_stop_px"] or age >= CTRL_CFG["hold_force_zero_ms"]:
                ty = 0.0
                self.slew_target_y = 0.0
...
class K230D36ALaserRectTrackerV10:
    ...
    def apply_slider(self, name, value):
        ...
        elif name in ("aimoffsetx", "aim_x", "aimx", "xaim"):
            CTRL_CFG["aim_offset_x"] = clamp(value, -80.0, 80.0)
            self.controller.reset()
        elif name in ("aimoffsety", "aim_y", "aimy", "yaim"):
            CTRL_CFG["aim_offset_y"] = clamp(value, -80.0, 80.0)
            self.controller.reset()
        elif name in ("xmicon", "xmicroenable", "x_micro_enable"):
            CTRL_CFG["x_micro_enable_px"] = clamp(value, 0.5, 20.0)
        elif name in ("ymicon", "ymicroenable", "y_micro_enable"):
            CTRL_CFG["y_micro_enable_px"] = clamp(value, 0.5, 20.0)
        elif name in ("xmicrohz", "x_micro_hz"):
            CTRL_CFG["x_micro_hz"] = clamp(value, 8.0, 80.0)
        elif name in ("ymicrohz", "y_micro_hz"):
            CTRL_CFG["y_micro_hz"] = clamp(value, 8.0, 80.0)
        elif name in ("xfinegain", "x_fine_gain"):
            CTRL_CFG["fine_i_gain_x"] = clamp(value, 0.0, 20.0)
            self.controller.fine_i_x = 0.0
        elif name in ("yfinegain", "y_fine_gain"):
            CTRL_CFG["fine_i_gain_y"] = clamp(value, 0.0, 20.0)
            self.controller.fine_i_y = 0.0
    ...
    def send_plot(self, rect_result, state_code):
        ...
        if rect_pos is not None and laser_pos is not None:
            raw_err_x = float(rect_pos[0] - laser_pos[0]) + float(CTRL_CFG["aim_offset_x"])
            raw_err_y = float(rect_pos[1] - laser_pos[1]) + float(CTRL_CFG["aim_offset_y"])
    ...
    def run(self):
        ...
        try:
            print("=" * 78)
            print("K230 D36A LASER-RECT TRACKER V10.11 SMOOTH-MOTION TUNE")
            ...
            self.uart.send("[system,ready,k230_d36a_laser_rect_v10_11_smooth_motion_tune]")
            self.uart.send("[display,8,8,v10.11 smooth: CH1/2 err CH3/4 targetHz CH5/6 outputHz,14]")
