# -*- coding: utf-8 -*-
"""
K230 + D36A 二维步进云台矩形-激光闭环追踪 v5.8（步进速度与精度优化版）
...
MOTION_CFG = {
    ...
    "x_accel_hz_s": 10000.0,
    "y_accel_hz_s": 8500.0,
    "x_decel_hz_s": 11000.0,
    "y_decel_hz_s": 9500.0,
    ...
    def run(self):
        ...
        try:
            ...
            if ENABLE_DISPLAY:
                ...
                display_ok = True
                print('Display ready: ST7701 640x480; IDE mirror ON')
            MediaManager.init()
            sensor.run()
            time.sleep_ms(300)
            ...
            while True:
                ...
                if display_ok and frame_count % max(1, DISPLAY_EVERY_N_FRAMES) == 0:
                    ...
                    try:
                        display_img = img.resize(640, 480)
                        Display.show_image(display_img, x=0, y=0)
                    except Exception:
