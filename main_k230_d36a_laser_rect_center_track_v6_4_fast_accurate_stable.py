# -*- coding: utf-8 -*-
"""
K230 + D36A 二维步进云台矩形-激光闭环追踪 v6.4（快速、精准、稳定综合优化版）
...
TRACK_CFG = {
    ...
    "max_coast_frames": 7,
    "keep_track_ms": 5000,
    "control_lead_s": 0.018,

    "gate_tracking_px": 125.0,
    "gate_reacquire_px": 240.0,
...
MOTION_CFG = {
    ...
    "x_accel_hz_s": 9000.0,
    "y_accel_hz_s": 10500.0,
    "x_decel_hz_s": 13000.0,
    "y_decel_hz_s": 14000.0,
    ...
    "x_mid_cap_hz": 900.0,
    "y_mid_cap_hz": 760.0,
    # 保留1800/1600 Hz硬件余量，正常闭环远区限制在更安全的1400/1150 Hz。
    "x_far_cap_hz": 1400.0,
    "y_far_cap_hz": 1150.0,
...
CONTROL_ERROR_ALPHA_NEAR = 0.48
CONTROL_ERROR_ALPHA_MID = 0.65
CONTROL_ERROR_ALPHA_FAR = 0.82
CONTROL_REL_VEL_ALPHA = 0.16
RECT_COAST_CMD_SCALE_2 = 0.42
RECT_COAST_CMD_SCALE_3 = 0.18
RECT_COAST_CMD_SCALE_4 = 0.08
RECT_COAST_CMD_SCALE_5 = 0.05
RECT_COAST_CMD_SCALE_6 = 0.02
RECT_COAST_MAX_FRAMES = 6
INVALID_HARD_RESET_FRAMES = 3
...
class LaserSpotTracker320:
    ...
    def detect(self, img, img_np, rect_box_det, desired_center_logical, dt):
        ...
        measured_det = self._subpixel_centroid_rgb565(
            img_np, measured_det[0], measured_det[1], blob_w, blob_h
        )
        self.last_area = area
...
# 单控制器优化版
# ============================================================
class K230D36ALaserRectTrackerV64:
    """v6.4 optimized: 单控制器激光点对准矩形中心版本。
...
状态机、软限位、诊断和异常清理，删除完整基础应用控制器与激光子类之间的重复实现。"""
    ...
    def _rect_coast_scale(miss_frames):
        ...
        if miss_frames == 3:
            return float(RECT_COAST_CMD_SCALE_3)
        if miss_frames == 4:
            return float(RECT_COAST_CMD_SCALE_4)
        if miss_frames == 5:
            return float(RECT_COAST_CMD_SCALE_5)
        if miss_frames == 6:
            return float(RECT_COAST_CMD_SCALE_6)
        return 0.0
    ...
    def _control_error_alpha(self, abs_error):
        """根据误差距离选择控制滤波强度。

        alpha越小滤波越强。小误差区抑制激光质心和矩形中心的帧间噪声，
        大误差区保留快速响应。
        """
        if abs_error <= float(PRECISION_CFG['precision_error_px']):
            return float(CONTROL_ERROR_ALPHA_NEAR)
        if abs_error <= float(PRECISION_CFG['mid_error_px']):
            return float(CONTROL_ERROR_ALPHA_MID)
        return float(CONTROL_ERROR_ALPHA_FAR)

    def _release_precision_hold_from_raw(self, raw_err_x, raw_err_y):
        """原始测量离开中心时立即解除停止保持。

        停止迟滞仍用于抗噪，但不再让滞后的滤波误差阻止电机响应
        已经明显移动的目标。
        """
        if self.x_precision_hold and abs(float(raw_err_x)) > float(PRECISION_CFG['x_stop_exit_px']):
            self.x_precision_hold = False
            self.filtered_err_x = float(raw_err_x)
            self.x_pid.reset()
        if self.y_precision_hold and abs(float(raw_err_y)) > float(PRECISION_CFG['y_stop_exit_px']):
            self.y_precision_hold = False
            self.filtered_err_y = float(raw_err_y)
            self.y_pid.reset()

    def _raw_error_command_cap(self, axis_name, raw_error, target):
        """用本帧原始误差给滤波控制量加安全上限。

        滤波误差用于抑制噪声，但大误差快速回到中心时它会滞后。
        此限幅防止真实误差已经很小时仍保留几百Hz指令。
        """
        ae = abs(float(raw_error))
        if axis_name == 'x':
            stop_enter = float(PRECISION_CFG['x_stop_enter_px'])
            near_cap = float(PRECISION_CFG['x_precision_cap_hz'])
            mid_cap = float(PRECISION_CFG['x_mid_cap_hz'])
            far_cap = float(MOTION_CFG['x_far_cap_hz'])
        else:
            stop_enter = float(PRECISION_CFG['y_stop_enter_px'])
            near_cap = float(PRECISION_CFG['y_precision_cap_hz'])
            mid_cap = float(PRECISION_CFG['y_mid_cap_hz'])
            far_cap = float(MOTION_CFG['y_far_cap_hz'])
        if ae <= stop_enter:
            return 0.0
        if ae <= float(PRECISION_CFG['precision_error_px']):
            cap = near_cap
        elif ae <= float(PRECISION_CFG['mid_error_px']):
            cap = mid_cap
        else:
            cap = far_cap
        return clamp(float(target), -cap, cap)
    ...
    def update_tracking_control(self, result, dt):
        ...
        self._release_precision_hold_from_raw(raw_err_x, raw_err_y)
        if self.filtered_err_x is None:
            self.filtered_err_x = raw_err_x
            self.filtered_err_y = raw_err_y
        else:
            ex_alpha = self._control_error_alpha(abs(raw_err_x))
            ey_alpha = self._control_error_alpha(abs(raw_err_y))
            # 目标突然远离中心或跨过中心时快速跟上原始测量；
            # 接近中心时仍保留原有强滤波。
            if raw_err_x * self.filtered_err_x <= 0.0 or abs(raw_err_x) > abs(self.filtered_err_x) + 6.0:
                ex_alpha = max(ex_alpha, 0.88)
            if raw_err_y * self.filtered_err_y <= 0.0 or abs(raw_err_y) > abs(self.filtered_err_y) + 6.0:
                ey_alpha = max(ey_alpha, 0.88)
            self.filtered_err_x += ex_alpha * (raw_err_x - self.filtered_err_x)
            self.filtered_err_y += ey_alpha * (raw_err_y - self.filtered_err_y)
        ...
        x_target = self._raw_error_command_cap('x', raw_err_x, x_target)
        y_target = self._raw_error_command_cap('y', raw_err_y, y_target)
        command_scale = 1.0
    ...
    def run(self):
        ...
        try:
            print('=' * 78)
            print('K230 D36A LASER-RECT TRACKER V6.4 FAST ACCURATE STABLE')
            ...
            if self.uart.is_ready():
                self.uart.send('[system,ready,k230_d36a_laser_rect_v6_4_fast_accurate_stable]')
            ...
            while True:
                ...
                if display_ok and frame_count % max(1, DISPLAY_EVERY_N_FRAMES) == 0:
                    ...
                    # 直接居中显示320x240图像，避免每次显示前执行640x480缩放和大帧分配。
...
if __name__ == "__main__":
    K230D36ALaserRectTrackerV64().run()
