...
class StepperAxis:
    ...
    def __init__(self, name, step_pin, pwm_channel, dir_pin, positive_dir_level, reverse,
        ...
        try:
            self.pwm = PWM(self.pwm_channel, int(PWM_INIT_FREQ_HZ), 0, enable=False)
        except TypeError:
            self.pwm = PWM(self.pwm_channel, int(PWM_INIT_FREQ_HZ), 0, False)
...
class IBVSSystemController:
    ...
    def handle_uart(self):
        for parts in self.uart.read_packets(UART_MAX_PACKETS_PER_LOOP):
            ...
            elif typ == "system" and cmd == "ping": self.uart.send("[system,pong,v9_4]")
    ...
    def run(self):
        try:
            print("=" * 78)
            print("K230 D36A LASER TARGET IBVS V9.4 PWM BLOB MOTION FIX")
            ...
            self.last_frame_ms = time.ticks_ms()
            self.fps = 0.0
            self.uart.send("[system,ready,k230_d36a_laser_target_ibvs_v9_4_pwm_blob_motion_fix]")
            while True:
                os.exitpoint()
