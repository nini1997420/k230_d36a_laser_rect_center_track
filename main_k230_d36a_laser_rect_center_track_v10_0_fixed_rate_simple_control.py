# -*- coding: utf-8 -*-
"""
K230 + D36A 二维步进云台 矩形中心-激光点闭环 v10.0
固定周期简化控制版 / Fixed-rate simple-control version
...
LASER_CFG = {
    ...
    "dynamic_gate_max_det": 44.0,
    "rect_roi_margin_det": 45.0,
...
MEASURE_HOLD_MS = 170           # 短时漏检：保留误差但衰减目标频率
MEASURE_LOST_MS = 360           # 超过该时间：目标频率归零，Ramp减速停
...
CTRL_CFG = {
    ...
    "x_start_error_px": 8.0,
    "y_start_error_px": 8.0,
            ...
            if self.center is not None:
                ...
                for c in candidates:
                    ...
                    if d <= gate and c["quality"] >= min_q:
                        cost = d - 70.0 * c["quality"]
                        if cost < best_cost:
                            chosen = c
                            best_cost = cost
            if chosen is None and candidates[0]["quality"] >= RECT_CFG["quality_recover"]:
                chosen = candidates[0]
...
class FixedRatePDController:
    ...
    def _axis_pd(self, axis_name, e, dt):
        ...
        # 小误差跨零后先停，不强行反向。
        if prev is not None and prev * e < 0.0 and ae < 12.0:
            target = 0.0
        return target
...
class K230D36ALaserRectTrackerV10:
    ...
    def run(self):
        ...
        try:
            print("=" * 78)
            print("K230 D36A LASER-RECT TRACKER V10.0 FIXED-RATE SIMPLE CONTROL")
            ...
            self.uart.send("[system,ready,k230_d36a_laser_rect_v10_0_fixed_rate]")
            self.uart.send("[display,8,8,v10: CH1/2 err CH3/4 targetHz CH5/6 outputHz CH9 state CH10 fps,14]")
