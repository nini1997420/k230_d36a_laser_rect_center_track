...
基础矩形层不初始化 GPIO35；v4.0整合层将GPIO35用于激光TTL。严禁把 D36A 的 5V 接到任何 K230 GPIO。
...
class K230RectangleStepperTrackerV31:
    def __init__(self):
        ...
        print("D36A EN1/EN2 must be tied to board 5V; GPIO35 reserved for laser TTL in v4.0")
    ...
    def _pause_motion_preserve_state(self):
        # 暂停STEP，但保留PID与上一误差，避免短时漏检后重新从零启动。
        self.last_x_target_hz = 0.0
        self.last_y_target_hz = 0.0
        self.x_axis.hard_stop()
        self.y_axis.hard_stop()
    ...
    def update_tracking_control(self, result, dt):
        ...
        if measured is None or state not in (STATE_TRACK, STATE_COAST):
            self.control_arm_count = 0
            self.invalid_control_frames += 1
            self.previous_error_norm = None
            if self.invalid_control_frames <= 1:
                self._pause_motion_preserve_state()
            else:
                self.stop_motion(hard=True)
        ...
        # 误差低通只抑制激光质心亚像素跳动，不对大误差做重滤波。
        a = float(CONTROL_ERROR_ALPHA)
        if self.filtered_err_x is None:
            self.filtered_err_x = raw_err_x
            self.filtered_err_y = raw_err_y
        else:
            self.filtered_err_x += a * (raw_err_x - self.filtered_err_x)
            self.filtered_err_y += a * (raw_err_y - self.filtered_err_y)
        err_x = self.filtered_err_x
        err_y = self.filtered_err_y
        ...
        raw_rel_vx = rect_vx - laser_vx
        raw_rel_vy = rect_vy - laser_vy
        ...
        x_target, _ = self.compute_axis_target(
            self.x_pid, err_x, self.filtered_rel_vx, dt,
        ...
        y_target, _ = self.compute_axis_target(
            self.y_pid, err_y, self.filtered_rel_vy, dt,
        ...
        # 矩形或激光仅处于1帧预测时，保留方向但降低速度。
        if state == STATE_COAST:
            x_target *= float(RECT_COAST_CMD_SCALE_1)
            y_target *= float(RECT_COAST_CMD_SCALE_1)
        if self.laser.last_source == "PREDICT":
            x_target *= float(LASER_COAST_CMD_SCALE_1)
            y_target *= float(LASER_COAST_CMD_SCALE_1)
    ...
    def run(self):
        ...
        try:
            ...
            print("EN1/EN2 tied to D36A board 5V; GPIO35 reserved for laser TTL in v4.0")
            print("=" * 76)
            self.startup_motor_self_test()
            ...
            # 通道0：矩形检测，保持v3.6已经验证准确的320x240 RGB888 + cv_lite。
            sensor.set_framesize(
                width=DETECT_WIDTH, height=DETECT_HEIGHT, chn=RECT_CHANNEL
            )
            sensor.set_pixformat(Sensor.RGB888, chn=RECT_CHANNEL)

            # 通道1：激光检测，固定320x240 RGB565。
            # 当前CanMV v1.4.3上，find_blobs的颜色/LAB路径应使用RGB565；
            # 旧v4.0在RGB888上调用find_blobs，导致laserCand长期为0。
            sensor.set_framesize(
                width=DETECT_WIDTH, height=DETECT_HEIGHT, chn=LASER_CHANNEL
            )
...
# 2) 矩形检测、Kalman、候选关联完全继承上面的 v3.6 稳定版本；
# 3) 新控制误差 = 矩形中心 - 激光点中心；
...
LASER_PREDICT_MAX_FRAMES = 1
LASER_PREDICT_VEL_DECAY = 0.72
LASER_COAST_CMD_SCALE_1 = 0.78
RECT_COAST_CMD_SCALE_1 = 0.82
CONTROL_ERROR_ALPHA = 0.70
CONTROL_REL_VEL_ALPHA = 0.22
RECT_COAST_CMD_SCALE_2 = 0.58
...
PRECISION_CFG = {
    ...
    "x_precision_cap_hz": 105.0,
    "y_precision_cap_hz": 82.0,
    "x_mid_cap_hz": 520.0,
    "y_mid_cap_hz": 380.0,
}
...
LASER_CFG = {
    ...
    "gate_locked_px_det": 18.0,
    "gate_reacquire_px_det": 64.0,
    "acquire_jump_px_det": 9.0,
    "max_lost_frames": 3,
    "acquire_frames": 3,
    ...
    "rect_roi_margin_det": 52.0,
    "roi_margin_det": 20.0,
    "full_scan_fallback_frames": 18,
}
...
class LaserSpotTracker320:
    ...
    def _build_roi(self, rect_box_det):
        ...
        # 目标矩形附近是激光最可能出现的位置。未锁定时也不再默认全图扫描。
        if rect_box_det is not None:
            x, y, w, h = rect_box_det
            m = float(LASER_CFG["rect_roi_margin_det"])
            boxes.append((x - m, y - m, x + w + m, y + h + m))

        reference = self.position_det if self.position_det is not None else self.provisional_det
        if reference is not None:
            g = float(LASER_CFG["gate_reacquire_px_det"])
            boxes.append((reference[0] - g, reference[1] - g,
                          reference[0] + g, reference[1] + g))

        # 连续找不到时，允许偶尔全图严格CORE搜索，但HALO永远不会全图参与首次捕获。
        fallback = self.lost_frames >= int(LASER_CFG["full_scan_fallback_frames"])
    ...
    def _handle_no_measurement(self, dt=0.0):
        ...
        # v4.3：只允许已锁定激光在单帧漏检时按速度短时预测。
        # 这样不会因为1帧颜色检测失败就把STEP硬切为0，随后又重新加速。
        if (
            self.locked
    ...
    def detect(self, img, img_np, rect_box_det, desired_center_logical, dt):
        ...
        # HALO只允许在已锁定且CORE暂时缺失时使用，绝不参与全图首次捕获。
        halo_blobs = []
        if self.locked and not core_blobs and self.position_det is not None:
            g = float(LASER_CFG["gate_locked_px_det"])
            hx, hy = self.position_det
        ...
        # 未锁定时以连续帧的临时位置为参考，不再以矩形中心吸引候选。
        reference = predicted_det if self.locked else self.provisional_det
        ...
        for blob in halo_blobs:
            c = self._candidate_from_blob(blob, "HALO", reference, float(LASER_CFG["gate_locked_px_det"]), img_np)
            if c is not None:
                candidates.append(c)
...
class K230D36ARectangleLaserTrackerV45(K230RectangleStepperTrackerV31):
    """v4.5：亚像素激光中心、三级速度控制和中心停止迟滞。"""
    ...
    def state_code(self, result):
        if self.direction_fault:
            return 4
        return super().state_code(result)
    ...
    def _measure_error(self, result):
        ...
        if rect is None or laser is None or not self.laser.locked:
            return None
        return (
            float(rect[0] - laser[0]),
            float(rect[1] - laser[1]),
        )
    ...
    def _check_divergence(self, result, err_x, err_y):
        ...
        if self.previous_error_norm is not None:
            grew = (
                norm > self.previous_error_norm + DIVERGENCE_GROW_PX
                and norm > self.previous_error_norm * DIVERGENCE_GROW_RATIO
            )
    ...
    def _precision_axis_target(self, axis_name, pid, error_px, velocity_px_s,
                               dt, previous_error, axis, ff_gain, ff_limit_hz,
                               far_cap_hz):
        ...
        try:
            ...
            target, braked = self.compute_axis_target(
                pid, error_px, velocity_px_s, dt,
                0.0, ff_gain * ff_scale, ff_limit_hz * ff_scale,
                axis.min_hz, zone_cap, previous_error,
                zone_cap, zone_cap, zone_cap,
            )
        finally:
            pid.kp = base_kp
            pid.kd = base_kd

        # 精定位区不允许速度噪声把输出推向误差反方向。
    ...
    def update_tracking_control(self, result, dt):
        ...
        self.invalid_control_frames = 0
        self.control_arm_count = min(
            self.control_arm_count + 1,
            int(LASER_CONTROL_ARM_FRAMES) + 1000,
        )
        if self.control_arm_count < int(LASER_CONTROL_ARM_FRAMES):
            self._pause_step_preserve_control()
            return
        ...
        # 误差已经经过矩形Kalman和激光位置滤波，这里仅做轻量平滑。
        ...
        x_target, self.last_x_zone = self._precision_axis_target(
            "x", self.x_pid, err_x, self.filtered_rel_vx, dt,
            self.last_err_x, self.x_axis,
            MOTION_CFG["x_ff"], MOTION_CFG["x_ff_limit_hz"],
            MOTION_CFG["x_far_cap_hz"],
        )
        y_target, self.last_y_zone = self._precision_axis_target(
            "y", self.y_pid, err_y, self.filtered_rel_vy, dt,
            self.last_err_y, self.y_axis,
            MOTION_CFG["y_ff"], MOTION_CFG["y_ff_limit_hz"],
            MOTION_CFG["y_far_cap_hz"],
        )

        # 矩形预测帧和激光预测帧都只能降速，不能在无新测量时加速。
        command_scale = 1.0
        if state == STATE_COAST:
            command_scale *= self._rect_coast_scale(
                int(result.get("miss_frames", 1))
            )
        if self.laser.last_source == "PREDICT":
    ...
    def update_coast_control(self, result, dt):
        if (
            not LASER_COAST_ENABLE
            or int(result.get("miss_frames", 0)) > int(RECT_COAST_MAX_FRAMES)
        ):
            self.control_arm_count = 0
            self.stop_motion(hard=True)
            return
    ...
    def apply_slider(self, name, raw_value):
        ...
        if n in ("laseron", "laser_on"):
            if value >= 0.5:
                ...
                self.current_laser_pos = None
                self.stop_motion(hard=True)
            self.uart.send("[ack,slider,%s,%d]" % (
                name, 1 if self.laser_output.enabled else 0
            ))
            return
        ...
        mapping = {
            # 兼容旧命令：laserlmin/amin/bmin现在修改严格CORE阈值。
            "laserlmin": ("core_l_min", 0, 100),
            "laserlmax": ("core_l_max", 0, 100),
            "laseramin": ("core_a_min", -128, 127),
            "laseramax": ("core_a_max", -128, 127),
            "laserbmin": ("core_b_min", -128, 127),
            "laserbmax": ("core_b_max", -128, 127),
            "laserhalolmin": ("halo_l_min", 0, 100),
            "laserhaloamin": ("halo_a_min", -128, 127),
            "laserhalobmin": ("halo_b_min", -128, 127),
            "laserminarea": ("area_min", 1, 200),
            "lasermaxarea": ("area_max", 2, 1000),
            "lasermindensity": ("min_density", 0.0, 1.0),
            "lasermaxw": ("max_w_det", 2, 100),
            "lasermaxh": ("max_h_det", 2, 100),
            "lasermaxaspect": ("max_aspect", 1.0, 10.0),
            "lasergate": ("gate_locked_px_det", 5, 100),
            "laseracquirejump": ("acquire_jump_px_det", 2, 80),
            "lasermaxcandidates": ("max_acquire_candidates", 1, 50),
            "laserambiguity": ("ambiguity_margin", 0.0, 0.5),
            "laserrectmargin": ("rect_roi_margin_det", 5, 160),
            "laserposalpha": ("position_alpha", 0.05, 1.0),
            "laservelalpha": ("velocity_alpha", 0.05, 1.0),
        }
        aliases = {
            "laser_l_min": "laserlmin", "laser_l_max": "laserlmax",
            "laser_a_min": "laseramin", "laser_a_max": "laseramax",
            "laser_b_min": "laserbmin", "laser_b_max": "laserbmax",
            "laser_area_min": "laserminarea", "laser_area_max": "lasermaxarea",
            "laser_min_density": "lasermindensity", "laser_gate": "lasergate",
            "laser_acquire_jump": "laseracquirejump",
            "laser_max_candidates": "lasermaxcandidates",
            "laser_ambiguity": "laserambiguity",
            "laser_rect_margin": "laserrectmargin",
        }
        n2 = aliases.get(n, n)
        if n2 in mapping:
        ...
        super().apply_slider(name, raw_value)
    ...
    def send_plot(self, result):
        ...
        if PLOT_MODE == 1:
            source_code = {
                "NONE": 0, "CORE": 1, "HALO": 2, "PREDICT": 3,
            }.get(str(self.laser.last_source), 0)
            self.uart.send(
                "[plot,%d,%d,%d,%d,%d,%.3f,%d,%d,%d,%.2f]" % (
                    int(self.last_dt_ms),
                    int(result.get("miss_frames", 0)),
                    int(self.laser.lost_frames),
                    int(self.laser.raw_blob_count),
                    int(self.laser.candidate_count),
                    float(self.laser.confidence),
                    int(source_code),
                    int(self.control_arm_count),
                    int(state_code),
                    self.fps,
                )
            )
        ...
        if PLOT_MODE == 2:
            self.uart.send(
                "[plot,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%d,%d,%d,%.2f]" % (
                    self.x_axis.target_hz, self.y_axis.target_hz,
                    self.x_axis.applied_hz, self.y_axis.applied_hz,
                    self.x_axis.virtual_steps, self.y_axis.virtual_steps,
                    int(self.x_axis.limit_hit), int(self.y_axis.limit_hit),
                    int(state_code), self.fps,
                )
            )
            return
        ...
        if PLOT_MODE == 3:
            ...
            self.uart.send(
                "[plot,%.2f,%.2f,%.2f,%.2f,%.1f,%.1f,%.3f,%d,%d,%.2f]" % (
                    raw_err_x, raw_err_y,
                    float(filt_x), float(filt_y),
                    float(self.filtered_rel_vx), float(self.filtered_rel_vy),
                    float(self.laser.confidence),
                    int(source_code), int(state_code), self.fps,
                )
            )
            return
        ...
        self.uart.send(
            "[plot,%.2f,%.2f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%d,%.2f]" % (
                ...
                self.x_axis.target_hz, self.y_axis.target_hz,
                self.x_axis.applied_hz, self.y_axis.applied_hz,
                rect_x, rect_y, state_code, self.fps,
            )
        )
    ...
    def send_laser_packet(self):
        ...
        self.uart.send(
            "[laser,%.2f,%.2f,%.2f,%.2f,%.3f,%d,%.1f,%.3f,%d,%d]" % (
                lx, ly, vx, vy, self.laser.confidence,
                self.laser.candidate_count, self.laser.last_area,
                self.laser.last_density, self.laser.lost_frames,
                1 if self.laser_output.enabled else 0,
            )
        )

    def draw(self, img, result):
        # 所有候选矩形：蓝色细框。即使未最终锁定，也能判断检测器是否工作。
        for c in (result.get("candidates") or [])[:12]:
            try:
                x, y, w, h = c["box_det"]
                img.draw_rectangle(
                    int(x), int(y), int(w), int(h),
                    color=(40, 120, 255), thickness=1,
                )
            except Exception:
                pass

        # 最终目标矩形：TRACK绿，ACQUIRE黄，COAST橙，其余红。
        state_color = {
            STATE_TRACK: (0, 255, 0),
            STATE_ACQUIRE: (255, 255, 0),
            STATE_COAST: (255, 140, 0),
            STATE_LOST: (255, 0, 0),
            STATE_SEARCH: (255, 0, 0),
        }.get(result.get("state"), (255, 255, 0))
        ...
        if rect is not None:
            rect_det = (
                int(round(logical_to_detect_x(rect[0]))),
                int(round(logical_to_detect_y(rect[1]))),
            )
            img.draw_cross(rect_det[0], rect_det[1], color=(0, 255, 0), size=7, thickness=2)
        # 激光检测候选（来自独立RGB565通道）映射到同尺寸矩形画面。
        ...
        line1 = "%s R=%d L=%d/%d ARM=%d/%d%s" % (
            STATE_NAME[result.get("state", STATE_SEARCH)],
            1 if rect is not None else 0,
            1 if laser is not None else 0,
            1 if self.laser.locked else 0,
            arm, int(LASER_CONTROL_ARM_FRAMES), fault,
        )
        line2 = "e=(%.0f,%.0f) Hz=(%.0f,%.0f) Z=%d/%d RC=%d LC=%d/%d A=%d IO=%d/%d fps=%.1f" % (
            ex, ey, self.x_axis.applied_hz, self.y_axis.applied_hz,
            int(self.last_x_zone), int(self.last_y_zone),
            int(result.get("candidate_count", 0)),
            int(self.laser.candidate_count),
            int(self.laser.raw_blob_count),
            int(self.laser.ambiguous),
            1 if self.laser_output.enabled else 0,
            int(self.laser_output.active_level),
            self.fps,
        )
        try:
    ...
    def print_status(self, result):
        ...
        if rect is None or laser is None:
            print(
                "STAT state=%s rect=%d laser=%d rectCand=%d laserCand=%d/%d "
                "core=%d halo=%d amb=%d arm=%d/%d outHz=(%.0f,%.0f) "
                "io=%d/A%d ferr=%d fps=%.1f" % (
                    STATE_NAME[result.get("state", STATE_SEARCH)],
                    1 if rect is not None else 0,
                    1 if laser is not None else 0,
                    int(result.get("candidate_count", 0)),
                    int(self.laser.candidate_count),
                    int(self.laser.raw_blob_count),
                    int(self.laser.core_blob_count),
                    int(self.laser.halo_blob_count),
                    int(self.laser.ambiguous),
                    self.control_arm_count, int(LASER_CONTROL_ARM_FRAMES),
                    self.x_axis.applied_hz, self.y_axis.applied_hz,
                    1 if self.laser_output.enabled else 0,
                    int(self.laser_output.active_level),
                    int(self.laser.find_error_count),
                    self.fps,
                )
            )
        else:
            print(
                "STAT state=%s err=(%.1f,%.1f) rect=(%.1f,%.1f) "
                "laser=(%.1f,%.1f) rectCand=%d laserCand=%d/%d src=%s amb=%d arm=%d/%d "
                "outHz=(%.0f,%.0f) io=%d/A%d fps=%.1f" % (
                    STATE_NAME[result.get("state", STATE_SEARCH)],
                    rect[0] - laser[0], rect[1] - laser[1],
                    rect[0], rect[1], laser[0], laser[1],
                    int(result.get("candidate_count", 0)),
                    int(self.laser.candidate_count),
                    int(self.laser.raw_blob_count),
                    str(self.laser.last_source),
                    int(self.laser.ambiguous),
                    self.control_arm_count, int(LASER_CONTROL_ARM_FRAMES),
                    self.x_axis.applied_hz, self.y_axis.applied_hz,
                    1 if self.laser_output.enabled else 0,
    ...
    def run(self):
        ...
        try:
            print("=" * 78)
            print("K230 D36A RECTANGLE-LASER TRACKER V4.5 PRECISION-CONTROL")
            ...
            print("Precision control: synchronized subpixel center + 3-zone speed + coast continuity")
            print("=" * 78)

            # 启动时不自动运动。需要检查接线时，通过串口发送 [motor,test]。
            ...
            # 通道1：激光检测，使用320x240 RGB565。
            # 当前CanMV v1.4.3实测RGB888颜色LAB链路会使find_blobs长期0候选。
            sensor.set_framesize(
                width=DETECT_WIDTH, height=DETECT_HEIGHT, chn=LASER_CHANNEL
            )
            sensor.set_pixformat(Sensor.RGB565, chn=LASER_CHANNEL)
            ...
            if LASER_AUTO_ON_AFTER_CAMERA:
                self.laser_output.on()
                print(
                    "Camera ready; RECT ch%d RGB888 + LASER ch%d RGB565; "
                    "laser GPIO35 ON; searching rectangle and laser"
                    % (RECT_CHANNEL, LASER_CHANNEL)
                )

            if self.uart.is_ready():
                self.uart.send("[system,ready,k230_d36a_rect_laser_v4_5]")
            ...
            while True:
                ...
                # 矩形帧：RGB888，供cv_lite、Kalman与屏幕叠加。
                img = sensor.snapshot(chn=RECT_CHANNEL)
                img_np = img.to_numpy_ref()
                result = self.tracker.step(img, img_np, dt)

                # 激光帧：RGB565，专供find_blobs颜色检测。
...
if __name__ == "__main__":
    K230D36ARectangleLaserTrackerV45().run()
