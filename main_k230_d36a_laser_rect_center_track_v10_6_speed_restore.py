# -*- coding: utf-8 -*-
"""
K230 + D36A 二维步进云台 矩形中心-激光点闭环 v10.6
速度恢复修正版 / Speed restore + micro-nudge version
...
RECT_CFG = {
    ...
    # v10.6：强目标身份锁定。v10数据中的乱抖主要来自矩形候选跳到远处干扰框，
...
MEASURE_FRESH_MS = 120           # 两个测量都在此时间内有效才全速TRACK
MEASURE_HOLD_MS = 420           # 短时漏检：保留误差但衰减目标频率
MEASURE_LOST_MS = 850           # 超过该时间：目标频率归零，Ramp减速停
...
CTRL_CFG = {
    ...
    # v10.6：低死区精修。目标是消除 3~6 px 残余偏差；代价是中心附近可能更活跃。
    "x_stop_enter_px": 0.45,
    "x_stop_exit_px": 0.95,
    "y_stop_enter_px": 0.45,
    "y_stop_exit_px": 0.95,
    ...
    # v10.6：误差超过约0.75px就允许进入微调判定。
    "x_start_error_px": 0.75,
    "y_start_error_px": 0.75,

    # v10.6：微调强制起步。只补足小输出，不再钳制大误差下的正常PD速度。
    # 只要原始误差达到该门槛，就给一个可实际转动的低速脉冲。
    "x_micro_enable_px": 1.20,
    "y_micro_enable_px": 1.20,
    "x_micro_hz": 34.0,
    "y_micro_hz": 32.0,
    "micro_same_sign_px": 2.20,
    ...
    # v10.6：测量异常跳变保护。第一帧大跳变先当作可疑测量，只衰减输出；
    ...
    # v10.6：目标频率二级限斜率。StepperAxis已有Ramp，这里再限制控制目标的突然变化，
    # 让 target_hz 本身连续，减少重捕获/短时漏检后的冲击。
    "x_target_slew_hz_s": 9000.0,
    "y_target_slew_hz_s": 7800.0,
    "x_zero_cross_stop_px": 0.65,
    "y_zero_cross_stop_px": 0.65,
...
class FixedRatePDController:
    ...
    def _axis_pd(self, axis_name, e, dt):
        ...
        # 小误差跨零后只在约2px内短暂停；v10.6继续缩小跨零停区，减少最终偏差。
    ...
    def _apply_micro_nudge(self, axis_name, target, raw_e, filt_e):
        # 只用于有效测量帧：放宽“需要转动电机”的条件。
        # v10.6 修复点：微调逻辑只能补足“小输出”，不能把大误差下的正常PD输出
        # 错误地钳位到 near_cap。v10.5 的速度过慢主要来自这里。
        if axis_name == "x":
...
class K230D36ALaserRectTrackerV10:
    ...
    def run(self):
        ...
        try:
            print("=" * 78)
            print("K230 D36A LASER-RECT TRACKER V10.3 PRECISION-CENTER")
            ...
            self.uart.send("[system,ready,k230_d36a_laser_rect_v10_6_speed_restore]")
            self.uart.send("[display,8,8,v10.6 speed-restore: CH1/2 err CH3/4 targetHz CH5/6 outputHz,14]")
