...
class RectangleTracker:
    ...
    def associate(self, candidates, reacquire=False, low_stage=False):
        ...
        for c in candidates:
            ...
            # v4.6：锁定后拒绝同时缺少黑白对比、内外框结构且质量一般的候选。
...
LASER_CFG = {
    ...
    "gate_locked_px_det": 20.0,
    "gate_reacquire_px_det": 34.0,
...
class LaserSpotTracker320:
    ...
    def detect(self, img, img_np, rect_box_det, desired_center_logical, dt):
        ...
        # v4.6：锁定或刚丢锁时始终在预测/锚点附近补做HALO搜索。
...
class K230D36ARectangleLaserTrackerV46(K230RectangleStepperTrackerV31):
    """v4.6：修复软限位阻塞、激光重捕获跳点和矩形短时误关联。"""
    ...
    def _update_auto_rebase(self, err_x, err_y, state):
        if not AUTO_REBASE_ENABLE or state != STATE_TRACK or self.laser.last_source == "PREDICT":
            ...
            return
        stable = (
            abs(err_x) <= AUTO_REBASE_ERR_X_PX
            and abs(err_y) <= AUTO_REBASE_ERR_Y_PX
            and abs(self.x_axis.applied_hz) < self.x_axis.min_hz
            and abs(self.y_axis.applied_hz) < self.y_axis.min_hz
        )
        if stable:
    ...
    def run(self):
        ...
        try:
            print("=" * 78)
            print("K230 D36A RECTANGLE-LASER TRACKER V4.6 FAST-STABLE")
            ...
            print("Fast-stable: strict rectangle association + anchored laser reacquire + visual limit guard")
            print("=" * 78)
            ...
            if self.uart.is_ready():
                self.uart.send("[system,ready,k230_d36a_rect_laser_v4_6]")
...
if __name__ == "__main__":
    K230D36ARectangleLaserTrackerV46().run()
